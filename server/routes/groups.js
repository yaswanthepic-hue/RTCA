...
router.post('/', auth, async (req, res) => {
  try {
    ...
    for (const u of users) {
      if (u.isPrivate) {
        // Private account: send an invite, they must accept to join the group (and can then be messaged)
        invites.push({
          group: null, // set after group creation
        ...
        // Public account but restricts group adds: needs approval, shows in their requests tab
        ...
        // Public, no restriction: add directly
    ...
    // Now persist invites with correct group ref
    if (invites.length > 0) {
    ...
    // Notify directly-added members and invited users via socket
